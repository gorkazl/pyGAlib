...
F. Klimm, J. Borge-Holthoefer, N. Wessel, J. Kurths, G. Zamora-López (2014)
...
def MaxDispersionCurve(d, M):
    ...
    M = float(M)
    x = 0.5 * (2.0 - d)
    return 1.0 - np.sqrt( M/(M-1.0) * ( x**2 + (1.0 - x)**2 - 1.0/M ) )


############################################################################
# 0) READ THE DATA OF THE CORTICAL NETWORK OF THE CAT
dataroot = 'Data/'
net = np.loadtxt(dataroot + 'Cat53_cortex.txt', np.uint8)
net = SymmetriseMatrix(net)
N = len(net)

# Define the partition of cat cortical network into functional subdivision
visual = np.arange(16)
auditory = np.arange(16,23)
somatomotor = np.arange(23,39)
frontolimbic = np.arange(39, 53)
